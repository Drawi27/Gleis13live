...
function showView(v) {
  ...
  if (v === 'mixes') {
    ...
    loadSets().then(() => loadArtistMeta()).then(() => { if (!SETS.length) SETS = DEMO_SETS; renderArtists(); });
  } else if (v === 'info') {
    ...
    // load dynamic info text from Supabase
    if (sb) sb.from('site_info').select('*').eq('id', 'main').single().then(({ data }) => {
      if (!data) return;
...
let _artistMetaCache = {};
async function loadArtistMeta() {
  if (!sb) return;
  try {
    const { data } = await sb.from('artists').select('*');
    if (data) {
      _artistMetaCache = {};
      data.forEach(a => { _artistMetaCache[a.name] = a; });
    }
  } catch(e) {}
}
...
function getArtistData() {
  ...
  SETS.forEach(s => {
    const key = s.artist;
    if (!map[key]) map[key] = { name: key, sets: [], ...(_artistMetaCache[key] || {}) };
    map[key].sets.push(s);
  });
  return Object.values(map).sort((a,b) => a.name.localeCompare(b.name));
}

async function saveArtistMeta(name, meta) {
  _artistMetaCache[name] = { ...(_artistMetaCache[name] || {}), ...meta, name };
  try {
    if (sb) await sb.from('artists').upsert({ name, ...meta });
  } catch(e) { console.warn('saveArtistMeta error:', e); }
}
...
async function loadInfoText() {
  if (!sb) return;
  try {
    const { data } = await sb.from('site_info').select('*').eq('id', 'main').single();
    if (!data) return;
...
async function saveInfoText() {
  if (!sb) { toast("Keine Datenbankverbindung"); return; }
  ...
  try {
    ...
    const { error } = await sb.from('site_info').upsert(row);
    if (error) { msg.textContent = 'Fehler: ' + error.message; msg.className = 'status-msg err'; return; }
    applyCustomTicker();
    msg.textContent = '✓ Gespeichert';
    msg.className = 'status-msg ok';
    setTimeout(() => msg.textContent = '', 2500);
  } catch(e) { msg.textContent = 'Fehler'; msg.className = 'status-msg err'; }
}
...
/* ═══════════════════════════════
   SUPABASE CONFIG
═══════════════════════════════ */
const SUPABASE_URL = 'https://gwelursuixzajquctdbj.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imd3ZWx1cnN1aXh6YWpxdWN0ZGJqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxNzg1NDMsImV4cCI6MjA4ODc1NDU0M30.08QbJkQmDH_rlshHDUa8Ea5-cgG-2AJLz-hVmdln2vc';
let sb = null;
try {
  if (typeof supabase !== 'undefined') {
    sb = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
  }
} catch(e) { console.warn('Supabase not available, using local fallback'); }
...
async function loadSets() {
  if (sb) {
    try {
      const { data, error } = await sb.from('sets').select('*').order('date', { ascending: false });
    ...
    } catch(e) { console.warn('Supabase loadSets error:', e); }
  }
  // Fallback: try localStorage
  try {
    const raw = localStorage.getItem('gleis13-sets');
    if (raw) { const parsed = JSON.parse(raw); if (parsed.length) { SETS = parsed; return; } }
  } catch(e) {}
  if (!SETS.length) SETS = [...DEMO_SETS];
}
...
async function loadGenres() {
  if (!sb) return;
  try {
    const { data } = await sb.from('genres').select('name');
    if (data) data.forEach(g => { if (!ALL_GENRES.includes(g.name)) ALL_GENRES.push(g.name); });
  } catch(e) {}
}
...
async function saveSets(setObj) {
  const row = {
    id: setObj.id,
    soundcloud_url: setObj.soundcloud_url,
    title: setObj.title,
    artist: setObj.artist,
    genres: setObj.genres || [],
    genre: setObj.genre || (setObj.genres?.[0] || ''),
    duration: setObj.duration || '',
    date: setObj.date || '',
    tags: setObj.tags || [],
    color: setObj.color || '#d63c1f',
    image: setObj.image || '',
  };
  // Update local array
  const idx = SETS.findIndex(s => s.id === setObj.id);
  if (idx >= 0) SETS[idx] = { ...SETS[idx], ...row }; else SETS.unshift(row);
  if (sb) {
    try {
      const { error } = await sb.from('sets').upsert(row);
      if (error) { console.error('saveSets error:', error); return false; }
      await saveGenres();
...
async function deleteSetFromApi(id) {
  ...
  if (sb) {
    try {
      ...
      if (error) { console.error('deleteSet error:', error); return false; }
...
async function saveGenres() {
  ...
  try {
    ...
    if (customGenres.length) {
      await sb.from('genres').upsert(customGenres.map(name => ({ name })));
    }
  } catch(e) {}
}
...
async function loadAndRender() {
  // Timeout: if loadSets takes more than 5 seconds, use demo data
  const loadPromise = loadSets();
  ...
  document.getElementById('grid').innerHTML = sets.map(s => {
    ...
    return `<div class="card${isOpen ? ' open' : ''}" id="card-${idx}">
      ...
      <div class="card-player">
        ${platform === 'youtube' ? `
          <div id="yt-container-${idx}" style="position:relative;background:#000">
            <div style="position:relative;padding-bottom:56.25%">
              <iframe id="yt-iframe-${idx}" style="position:absolute;inset:0;width:100%;height:100%;border:none"
                allow="autoplay;encrypted-media;picture-in-picture" allowfullscreen src=""></iframe>
            </div>
          </div>
        ` : `
...
function spShow(idx) {
  ...
  if (!s) return;
  const platform = getPlatform(s.soundcloud_url);
  if (platform === 'youtube') return; // YouTube plays in card, no sticky player
...
function spExpand() {
  ...
  const s = SETS[spIdx];
  const platform = getPlatform(s.soundcloud_url);
  if (platform === 'youtube') return; // YouTube plays in card
...
async function loadUserFavs() {
  if (!CURRENT_USER_ID || !sb) { _userFavsCache = []; return; }
  try {
    const { data } = await sb.from('favorites').select('set_id').eq('user_id', CURRENT_USER_ID);
    _userFavsCache = data ? data.map(f => f.set_id) : [];
  } catch(e) {}
}

function getUserFavsSync() {
  return _userFavsCache;
}

async function toggleFavInDb(setId, add) {
  if (!CURRENT_USER_ID || !sb) return;
  try {
    if (add) {
      await sb.from('favorites').upsert({ user_id: CURRENT_USER_ID, set_id: setId });
      if (!_userFavsCache.includes(setId)) _userFavsCache.push(setId);
    } else {
      await sb.from('favorites').delete().eq('user_id', CURRENT_USER_ID).eq('set_id', setId);
      _userFavsCache = _userFavsCache.filter(f => f !== setId);
    }
  } catch(e) {}
}
...
async function doUserLogin() {
  const nameOrEmail = document.getElementById('u-login-name').value.trim();
  const pw = document.getElementById('u-login-pw').value;
  const msg = document.getElementById('user-msg');
  if (!nameOrEmail || !pw) { msg.textContent = 'Bitte ausfüllen'; msg.className = 'status-msg err'; return; }
  msg.textContent = 'Einloggen...'; msg.className = 'status-msg';
  if (!sb) { msg.textContent = 'Keine Verbindung zur Datenbank'; msg.className = 'status-msg err'; return; }
  const email = nameOrEmail.includes('@') ? nameOrEmail : nameOrEmail + '@gleis13.de';
  const { data, error } = await sb.auth.signInWithPassword({ email, password: pw });
  if (error) {
    msg.textContent = error.message === 'Invalid login credentials' ? 'Falsches Passwort oder User nicht gefunden' : error.message;
    msg.className = 'status-msg err'; return;
  }
  CURRENT_USER = data.user.user_metadata?.display_name || nameOrEmail;
  CURRENT_USER_ID = data.user.id;
  IS_ADMIN = !!data.user.user_metadata?.is_admin;
  await loadUserFavs();
  closeUserModal(); updateUserUI();
  toast('\u2713 Willkommen, ' + CURRENT_USER);
...
async function doUserRegister() {
  ...
  if (name.length < 2) { msg.textContent = 'Min. 2 Zeichen'; msg.className = 'status-msg err'; return; }
  if (pw.length < 6) { msg.textContent = 'Min. 6 Zeichen Passwort'; msg.className = 'status-msg err'; return; }
  msg.textContent = 'Account erstellen...'; msg.className = 'status-msg';
  if (!sb) { msg.textContent = 'Keine Verbindung zur Datenbank'; msg.className = 'status-msg err'; return; }
  const email = name.includes('@') ? name : name + '@gleis13.de';
  const { data, error } = await sb.auth.signUp({ email, password: pw, options: { data: { display_name: name, is_admin: false } } });
  if (error) { msg.textContent = error.message; msg.className = 'status-msg err'; return; }
  CURRENT_USER = name;
...
function showUserMenu() {
  ...
  const favCount = _userFavsCache.length;
  ...
  document.getElementById('um-body').innerHTML = '<div style="display:flex;align-items:center;gap:1rem;padding-bottom:.75rem;border-bottom:1px solid #ddd"><div style="width:44px;height:44px;background:' + (IS_ADMIN ? '#f5c800' : 'var(--fg)') + ';color:' + (IS_ADMIN ? 'var(--fg)' : 'var(--bg)') + ';display:flex;align-items:center;justify-content:center;font-family:var(--display);font-size:1.4rem">' + CURRENT_USER[0].toUpperCase() + '</div><div><div style="font-family:var(--display);font-size:1.1rem;letter-spacing:.04em">' + CURRENT_USER + '</div><div style="font-size:.54rem;letter-spacing:.12em;text-transform:uppercase;color:var(--grey)">' + favCount + ' Favorit' + (favCount !== 1 ? 'en' : '') + (IS_ADMIN ? ' \u00b7 Admin' : '') + '</div></div></div>' + adminBtn + '<button onclick="closeUserModal();showView(\'favs\')" style="background:none;border:var(--b);color:var(--fg);padding:.6rem;font-family:var(--mono);font-size:.6rem;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;width:100%">\u2665 Meine Favoriten</button><button onclick="doUserLogout()" style="background:none;border:1px solid #ccc;color:var(--grey);padding:.6rem;font-family:var(--mono);font-size:.6rem;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;width:100%">Ausloggen</button>';
  modal.style.display = 'flex';
}

async function doUserLogout() {
  if (sb) await sb.auth.signOut();
  CURRENT_USER = null; CURRENT_USER_ID = null; IS_ADMIN = false; _userFavsCache = [];
  closeUserModal(); updateUserUI(); toast('Ausgeloggt');
  if (!document.getElementById('view-main').classList.contains('hidden')) renderGrid(activeFilter);
}
...
async function toggleFav(id) {
  ...
  const isCurrentlyFav = _userFavsCache.includes(id);
  await toggleFavInDb(id, !isCurrentlyFav);
  renderGrid(activeFilter);
  if (!document.getElementById('view-favs').classList.contains('hidden')) renderFavGrid();
}

function isFav(id) {
  return _userFavsCache.includes(id);
}

function renderFavGrid() {
  const favSets = SETS.filter(s => _userFavsCache.includes(s.id));
  ...
  document.getElementById('fav-grid').innerHTML = favSets.map(s => {
    ...
    const eTags = (s.tags || []).map(t => '<span class="tag">' + t + '</span>').join('');
    const slug = (s.soundcloud_url || '').replace(/^https?:\/\//, '');
    return '<div class="card" id="fav-card-' + idx + '"><div class="chrome"><div class="dots"><div class="dot"></div><div class="dot"></div></div><div class="chrome-url">' + slug + '</div><div class="chrome-date">' + fmtDate(s.date) + '</div></div>' + (s.image ? '<div style="position:relative;padding-bottom:56.25%;background:url(' + s.image + ') center/cover no-repeat;border-bottom:var(--b)"></div>' : '') + '<div class="card-body" onclick="showView(\'mixes\')"><div class="stripe" style="background:' + (s.color || '#333') + '"></div><div class="card-content"><div class="title">' + s.title + '</div><div class="artist">' + s.artist + '</div></div><div class="card-side"><button class="fav-btn liked" onclick="event.stopPropagation();toggleFav(\'' + s.id + '\')">\u2665</button><div class="dur">' + (s.duration || '') + '</div></div></div></div>';
  }).join('');
}
...
async function applyCustomTicker() {
  if (!sb) return;
  try {
    const { data } = await sb.from('site_info').select('ticker').eq('id', 'main').single();
...
// Restore Supabase session (non-blocking)
(async function initSession() {
  try {
    if (!sb) return;
    const { data: { session } } = await sb.auth.getSession();
    if (session?.user) {
      CURRENT_USER = session.user.user_metadata?.display_name || session.user.email?.split('@')[0] || 'User';
...
// Boot: always dismiss splash after max 4 seconds, even if Supabase hangs
setTimeout(() => {
  // Force dismiss splash no matter what after 4s
  dismissSplash();
}, 4000);
...
setTimeout(async () => {
  try {
    ...
    console.warn('loadAndRender failed:', e);
    SETS = [...DEMO_SETS];
    renderGrid('all');
  }
  try { applyCustomTicker(); } catch(e) {}
